...
// --- 4. MAIN DASHBOARD ---
export default function StLeadDashboard() {
  ...
  // ✅ Get Staff Info (Ensure 'branch' is saved in localStorage during login)
  const user = JSON.parse(localStorage.getItem('user')) || { name: 'Unknown', branch: 'Headquarters' };

  // ✅ Fetch Data (Now sends BOTH staff name AND branch)
  useEffect(() => {
    if (user.name === 'Unknown') return;

    const fetchDashboardData = async () => {
        try {
            // 🔒 FIX: Added 'branch' to params
            const params = { 
                params: { 
                    staff: user.name,
                    branch: user.branch 
    ...
    fetchDashboardData();
  }, [user.name, user.branch]); // ✅ Reload if branch changes
      ...
      <div className="max-w-7xl mx-auto relative z-10 space-y-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end gap-6 border-b border-white/5 pb-8">
          <div>
            ...
            <p className="text-slate-500 font-medium">Tracking performance for <span className="text-emerald-400 font-bold">{user.name}</span> <span className="text-slate-600">({user.branch})</span>.</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="md:col-span-2 grid grid-cols-2 gap-6">
             <StatCard title="My Leads" value={stats.total} subtext="Assigned to me" icon={Users} color="text-indigo-400" delay={0.1} />
             <StatCard title="Pending Actions" value={stats.today} subtext="Today + Overdue" icon={Clock} color="text-amber-400" delay={0.2} />
          </div>
          <StatCard title="In Progress" value={stats.inprogress} icon={TrendingUp} color="text-blue-400" delay={0.3} />
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="lg:col-span-2 bg-white/[0.03] backdrop-blur-xl border border-white/10 p-8 rounded-3xl">
            <div className="flex justify-between items-center mb-8">
              <div>
                ...
                <p className="text-sm text-slate-500 mt-1">My lead distribution</p>
              </div>
            </div>
...
{/*         
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.7 }} className="bg-white/[0.03] backdrop-blur-xl border border-white/10 p-6 rounded-3xl flex flex-col">
              <div className="flex justify-between items-center mb-6">
                ...
                <span className="text-xs font-bold text-amber-400 bg-amber-500/10 px-2 py-1 rounded-lg">My Tasks</span>
              </div>
              ...
              <div className="flex-1 space-y-3 overflow-y-auto max-h-[300px] custom-scrollbar pr-2">
                {tasks.length > 0 ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-40 text-center">
                        ...
                        <p className="text-slate-600 text-xs">No pending tasks for you.</p>
                    </div>
                )}
              </div>
              ...
              >
                View My Leads <ArrowRight size={14} />
              </button>
          </motion.div> */}
