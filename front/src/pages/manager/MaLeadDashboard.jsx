...
import { 
  ...
  Filter, Plus, BarChart3, ArrowRight, X, Save,
  MoreHorizontal, Briefcase, DollarSign
} from 'lucide-react';
    ...
    <div className="h-64 flex items-end justify-between gap-4 px-4 pb-2 border-b border-white/5">
      {data.map((item, i) => (
        <div key={item.label} className="w-full flex flex-col items-center gap-2 group h-full justify-end">
          ...
          <motion.div 
            initial={{ height: 0 }} 
            animate={{ height: `${(item.count / max) * 80}%` }} 
...
// --- 5. MAIN DASHBOARD ---
export default function MaLeadDashboard() {
  ...
  // ✅ GET MANAGER INFO
  const user = JSON.parse(localStorage.getItem('user')) || { branch: 'Headquarters' }; // Fallback

  // ✅ FETCH DATA (With Branch Filter)
  useEffect(() => {
    const fetchDashboardData = async () => {
        try {
            // Pass branch as query parameter
            const params = { params: { branch: user.branch } };

            // 1. Fetch Stats
            const statsRes = await axios.get('http://127.0.0.1:5000/api/dashboard/stats', params);
            setStats(statsRes.data);

            // 2. Fetch Tasks
            const tasksRes = await axios.get('http://127.0.0.1:5000/api/dashboard/tasks', params);
    ...
    <div className="min-h-screen bg-[#030712] text-slate-300 font-sans p-8 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10 space-y-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end gap-6 border-b border-white/5 pb-8">
          <div>
            <h1 className="text-4xl font-bold text-white tracking-tight mb-2">Branch Dashboard</h1>
            <p className="text-slate-500 font-medium">Overview for <span className="text-indigo-400 font-bold">{user.branch}</span> Branch.</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="md:col-span-2 grid grid-cols-2 gap-6">
             <StatCard title="Total Leads" value={stats.total} subtext="Branch Database" icon={Users} color="text-indigo-400" delay={0.1} />
             <StatCard title="Pending Actions" value={stats.today} subtext="Requires Attention" icon={Clock} color="text-amber-400" delay={0.2} />
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="lg:col-span-2 bg-white/[0.03] backdrop-blur-xl border border-white/10 p-8 rounded-3xl">
            <div className="flex justify-between items-center mb-8">
              <div>
                ...
                <p className="text-sm text-slate-500 mt-1">Branch performance metrics</p>
              </div>
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.7 }} className="bg-white/[0.03] backdrop-blur-xl border border-white/10 p-6 rounded-3xl flex flex-col">
              <div className="flex justify-between items-center mb-6">
                ...
                <span className="text-xs font-bold text-amber-400 bg-amber-500/10 px-2 py-1 rounded-lg">Pending</span>
              </div>
              ...
              <div className="flex-1 space-y-3 overflow-y-auto max-h-[300px] custom-scrollbar pr-2">
                {tasks.length > 0 ? (
                    tasks.map((lead, i) => {
                      ...
                      return (
                        <div key={i} className="bg-white/5 border border-white/5 p-3 rounded-xl hover:bg-white/10 transition-colors group">
                          <div className="flex justify-between items-start mb-2">
                            ...
                            <div className="text-right">
                                <span className={`text-[10px] font-bold block ${isOverdue ? 'text-rose-400' : 'text-emerald-400'}`}>
                                    {isOverdue ? 'Overdue' : 'Today'}
                                </span>
                                <span className="text-[10px] font-mono text-slate-400">
                                    {new Date(lead.follow_up_date).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
                                </span>
                            </div>
                          </div>
                          <div className="flex gap-2 mt-3">
                            <button onClick={() => handleAction('call', lead.name)} className="flex-1 flex items-center justify-center gap-2 py-1.5 rounded-lg bg-emerald-500/10 text-emerald-400 text-xs font-bold hover:bg-emerald-500 hover:text-white transition-all"><Phone size={12} />{lead.phone}</button>
                            <button onClick={() => handleAction('email', lead.name)} className="flex-1 flex items-center justify-center gap-2 py-1.5 rounded-lg bg-indigo-500/10 text-indigo-400 text-xs font-bold hover:bg-indigo-500 hover:text-white transition-all"><Mail size={12} /> {lead.email}</button>
                          </div>
                        </div>
                      );
                    })
                    ...
                    <div className="flex flex-col items-center justify-center h-40 text-center">
                        ...
                        <p className="text-slate-600 text-xs">No pending branch tasks.</p>
                    </div>
                )}
              </div>
              ...
              >
                View Branch Leads <ArrowRight size={14} />
              </button>
          </motion.div>
