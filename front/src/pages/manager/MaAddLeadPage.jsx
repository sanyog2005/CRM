...
import { 
  ...
  CheckCircle2, ArrowLeft, Loader2, AlertCircle, Lock
} from 'lucide-react';
...
// --- MAIN PAGE COMPONENT ---
export default function MaAddLeadPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    source: 'Website', 
    branch: user.branch, // ✅ Locked to Manager's Branch
    staff: '', 
  ...
  // ✅ 1. Fetch Only Relevant Staff
  useEffect(() => {
    const fetchStaff = async () => {
        try {
            const res = await axios.get('http://127.0.0.1:5000/api/staff');
            
            // Filter: Only show staff in THIS Manager's branch
            const branchStaff = res.data
                .filter(s => s.branch === user.branch) // 🔒 Filter Logic
                .map(s => s.name);
            ...
            if (branchStaff.length > 0) {
                setFormData(prev => ({ ...prev, staff: branchStaff[0] }));
            }
        } catch (err) {
            setNotification({ message: "Failed to load team list", type: "error" });
        }
    };
    fetchStaff();
  }, [user.branch]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setNotification({ message: `Lead saved to ${user.branch} branch!`, type: 'success' });
      setTimeout(() => navigate('/manager/leads'), 1500); // Redirect to Manager Dashboard
    } catch (err) {
    ...
    <div className="min-h-screen bg-[#030712] text-slate-300 font-sans p-4 md:p-8 flex items-center justify-center">
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-4xl relative z-10">
        <div className="flex items-center justify-between mb-8">
          <div>
            <button onClick={() => navigate('/manager/leads')} className="flex items-center gap-2 text-slate-500 hover:text-white transition-colors mb-2 text-sm font-bold"><ArrowLeft size={16} /> Back to Dashboard</button>
            <h1 className="text-3xl font-bold text-white tracking-tight">Add New Lead</h1>
            <p className="text-slate-500">Adding lead to <span className="text-indigo-400 font-bold">{user.branch}</span> Branch.</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white/[0.03] border border-white/10 rounded-[2rem] p-6 md:p-10 shadow-2xl">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12">
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <InputGroup 
                    ...
                    disabled={true} // Manager cannot change this
                    onChange={() => {}} 
                />
              </div>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {/* 🔒 ONLY SHOWS STAFF FROM THIS BRANCH */}
                <InputGroup 
          ...
          <div className="mt-10 pt-6 border-t border-white/5 flex justify-end gap-4">
            <button type="button" onClick={() => navigate('/manager/leads')} className="px-8 py-3.5 rounded-xl font-bold text-slate-400 hover:bg-white/5">Cancel</button>
