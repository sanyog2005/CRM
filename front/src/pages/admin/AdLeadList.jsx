...
import * as XLSX from 'xlsx'; // ✅ Import XLSX for Import Logic
...
import { 
  ...
  AlertCircle, X, ArrowUpDown, Download, Upload,
  Loader2, Save, Building2, DollarSign, Clock, Mail, Bell, FileSpreadsheet
} from 'lucide-react';
    ...
    <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-[#0b0f19] border border-white/10 w-full max-w-lg rounded-3xl p-8 shadow-2xl relative">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white/5 p-4 rounded-2xl border border-white/5"><p className="text-xs text-slate-500 font-bold uppercase mb-1">Notes</p><p className="text-sm text-slate-300 italic">"{lead.notes || 'No notes'}"</p></div>
        </div>
    ...
    <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-[#0b0f19] border border-white/10 w-full max-w-md rounded-3xl p-8 shadow-2xl relative max-h-[90vh] overflow-y-auto custom-scrollbar">
        ...
        <form onSubmit={(e)=>{e.preventDefault(); onSave(formData)}} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4"><div><label className="text-xs font-bold text-slate-400 ml-1 mb-1 block">Status</label><select value={formData.status || 'New'} onChange={e => handleChange('status', e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none cursor-pointer bg-[#0b0f19]">{['New', 'Contacted', 'Qualified', 'Negotiation', 'Lost'].map(s => <option key={s} className="bg-slate-900">{s}</option>)}</select></div><div><label className="text-xs font-bold text-slate-400 ml-1 mb-1 block">Branch</label><select value={formData.branch || ''} onChange={e => handleChange('branch', e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none cursor-pointer bg-[#0b0f19]">{['New York', 'London', 'Tokyo', 'Paris'].map(b => <option key={b} className="bg-slate-900">{b}</option>)}</select></div></div>
...
const StatusBadge = ({ status }) => {
  ...
  return <span className={`px-2.5 py-1 rounded-lg text-xs font-bold border ${styles[status] || 'bg-slate-500/10 text-slate-400'}`}>{status}</span>;
};
...
// --- 8. MAIN COMPONENT ---
export default function AdLeadList() {
  ...
  const [filters, setFilters] = useState({ status: 'All', branch: 'All', staff: 'All' });
  ...
  const branches = [...new Set(leads.map(l => l.branch))];
  const staffMembers = [...new Set(leads.map(l => l.staff))];
  ...
  const handleBulkImport = async (importedData) => {
    ...
    try {
        await axios.post('http://127.0.0.1:5000/api/leads/bulk', importedData);
        fetchLeads(); // Refresh list
        showToast(`${importedData.length} leads imported successfully!`);
    } catch (err) {
  ...
  const filteredLeads = useMemo(() => {
    return leads.filter(lead => {
      ...
      const matchBranch = filters.branch === 'All' || (lead.branch || '') === filters.branch;
      const matchStaff = filters.staff === 'All' || (lead.staff || '') === filters.staff;
    ...
    <div className="min-h-screen bg-[#030712] text-slate-300 font-sans p-8 relative overflow-hidden" onClick={() => setActiveMenu(null)}>
      ...
      <div className="max-w-screen-2xl mx-auto relative z-10 space-y-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end gap-6 border-b border-white/5 pb-8">
          <div>
            <h1 className="text-3xl font-bold text-white tracking-tight mb-2">Lead Database</h1>
            <p className="text-slate-500 font-medium">Manage and track your potential customers.</p>
          </div>
        ...
        <div className="bg-white/[0.02] border border-white/5 p-4 rounded-2xl backdrop-blur-xl flex flex-col xl:flex-row gap-4 justify-between items-start xl:items-center">
          ...
          <div className="flex flex-wrap items-center gap-3 w-full xl:w-auto">
            ...
            <FilterDropdown icon={MapPin} label="Branch" options={branches} value={filters.branch} onChange={(val) => setFilters({...filters, branch: val})} />
                    ...
                    <motion.tr key={lead.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ delay: index * 0.05 }} className="group hover:bg-white/[0.04] transition-colors cursor-pointer" onClick={() => handleAction('View', lead)}>
                      ...
                      <td className="p-5">
                        <div className="flex items-center gap-3">
                          ...
                          <div><div className="font-bold text-slate-200 text-sm group-hover:text-white">{lead.name || 'Unknown'}</div><div className="text-xs text-slate-500 flex items-center gap-1"><Building2 size={10}/> {lead.company || 'N/A'}</div></div>
                        </div>
                      </td>
                      <td className="p-5">
                      ...
                      <td className="p-5"><span className="text-xs text-slate-400 bg-white/5 px-2 py-1 rounded border border-white/5">{lead.source}</span></td>
                      <td className="p-5"><div className="text-sm text-slate-300 flex items-center gap-2"><MapPin size={14} className="text-slate-500" /> {lead.branch}</div></td>
                      <td className="p-5"><div className="text-sm text-slate-300 flex items-center gap-2"><User size={14} className="text-slate-500" /> {lead.staff}</div></td>
                      <td className="p-5"><StatusBadge status={lead.status} /></td>
                      <td className="p-5"><div className="text-sm font-bold text-emerald-400 flex items-center gap-1"><DollarSign size={12}/> {lead.value}</div></td>
